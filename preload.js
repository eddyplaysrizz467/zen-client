...
contextBridge.exposeInMainWorld("aeroApi", {
  ...
  addFriend: (payload) => invoke("friends:add", payload),
  removeFriend: (friendId) => invoke("friends:remove", friendId),
  getFriendsHostInfo: () => invoke("friends:hostInfo"),
  getServerPluginsState: () => invoke("serverPlugins:state"),
  createServerPluginHostCode: (payload) => invoke("serverPlugins:createHostCode", payload),
