...
function drawEnso(png, cx, cy, baseRadius, seed) {
  ...
  // Brush stroke: vary radius + thickness by angle, and leave a small "opening".
  const start = 0.2 * Math.PI; // opening offset
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    // Feather edge occasionally (adds realism).
    if (i % 11 === 0) {
  ...
  for (let i = 0; i < png.data.length; i += 4) {
    const g = (rng() - 0.5) * 12;
    png.data[i] = Math.max(0, Math.min(255, png.data[i] + g));
    png.data[i + 1] = Math.max(0, Math.min(255, png.data[i + 1] + g));
    png.data[i + 2] = Math.max(0, Math.min(255, png.data[i + 2] + g));
  }
}
...
async function main() {
  ...
  // Background: charcoal with a gentle vignette.
  const cx = size / 2;
  const cy = size / 2;
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const dx = (x - cx) / cx;
      const dy = (y - cy) / cy;
      const v = Math.min(1, Math.sqrt(dx * dx + dy * dy));
      const base = 16 - Math.round(v * 8);
